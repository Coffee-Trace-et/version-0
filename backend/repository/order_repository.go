...
func (or *OrderRepository) AcceptOrderDriver(orderID, Role string) (domain.Order, error) {
	...
	_, err = or.database.Collection(or.collection).UpdateOne(context.Background(), bson.M{"_id": orderID}, update)
	if err != nil {
		return domain.Order{}, err
	}
	return order, nil
...
func (or *OrderRepository) UpdateDestinations(orderID, Role string, Destinations domain.Destinations) (domain.Order, error) {
	...
	_, err = or.database.Collection(or.collection).UpdateOne(context.Background(), bson.M{"_id": order }, update)
	if err != nil {
		return domain.Order{}, err
	}
	return order, nil
