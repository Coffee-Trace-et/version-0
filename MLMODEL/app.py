...
coffee_data1.columns = [' '.join([str(i) for i in col if pd.notna(i)]) for col in coffee_data1.columns]

coffee_data2 = pd.read_excel('./extracted_files/Coffee-2019.xlsx', header=[1, 2])
coffee_data2.columns = [' '.join([str(i) for i in col if pd.notna(i)]) for col in coffee_data2.columns]
...
merged_data['Closing Price_x'] = pd.to_numeric(merged_data['Closing Price_x'].str.replace(',', ''), errors='coerce')
merged_data['Opening Price_x'] = pd.to_numeric(merged_data['Opening Price_x'].str.replace(',', ''), errors='coerce')

# Prepare data for Prophet model
merged_data['ds'] = pd.to_datetime(merged_data['Trade Date'])
merged_data['y'] = merged_data['Closing Price_x']  # Assuming you want to use 'Closing Price_x'
